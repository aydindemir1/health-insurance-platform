...
available on `http://localhost:8080` with the development administrator
credentials declared in `compose.yaml`.

Development identities imported automatically by Keycloak:

| Username | Password | Realm role |
| --- | --- | --- |
| `hospital.user` | `hospital` | `HOSPITAL_USER` |
| `insurance.specialist` | `insurance` | `INSURANCE_SPECIALIST` |

These credentials are deliberately limited to local development and must never
