...
const postgres = run(["exec", "-T", "claims-billing-db", "sh", "-c",
  `psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -P pager=off -c "select c.id,c.status as claim_status,c.version as claim_version,i.id as invoice_id,i.status as invoice_status,i.version as invoice_version,(select count(*) from invoice_payments p where p.invoice_id=i.id) as payment_count from claims c join invoices i on i.claim_id=c.id where c.id='${claimId}'; select id,dateexecuted from databasechangelog order by orderexecuted; select constraint_name from information_schema.table_constraints where table_name in ('claims','invoices','invoice_payments','claim_search_outbox') and constraint_name like 'chk_%' order by constraint_name; select action,correlation_id,occurred_at from audit_records where aggregate_id in ('${claimId}',(select id from invoices where claim_id='${claimId}')) order by occurred_at; select count(*) as search_projection_outbox_rows from claim_search_outbox where claim_id='${claimId}';"`]);
